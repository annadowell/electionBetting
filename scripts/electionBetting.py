...
def CleanName(input):
    ...
    if firstName in df2019Constituency['MemberFirstName'].str.capitalize().values:
        #st.write('yay')
        filtered_df = df2019Constituency[df2019Constituency['MemberFirstName'].str.capitalize() == firstName]
        if secondName in filtered_df['MemberSurname'].str.capitalize().values:
            #st.write('yip')
